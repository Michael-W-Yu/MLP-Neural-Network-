...
class SingleLayerNet():
    ...
    def fit(self, features, y, n_epochs=10000, lr=0.0001, mini_batch_sz=256, reg=0, verbose=2):
        ...
        self.wts = np.random.normal(loc = 0.0, scale = 0.01, size = (num_features, num_classes))
        self.b = np.random.normal(loc = 0.0, scale = 0.01, size = (num_classes, ))
        ...
        for i in range(n_epochs*iter_per_epoch):
            ...
            y_batch = y[index]
            net_act = self.activation(self.net_in(features_batch))
            loss = self.loss(self.net_in(features_batch), y_batch, reg)
            loss_history.append(loss)
            wtsGrad, biasGrad = self.gradient(features_batch, net_act, self.one_hot(y_batch, num_classes),reg)
            self.b = self.b - lr*biasGrad
            self.wts = self.wts - lr*wtsGrad
...
class SingleLayerNetSoftmax(SingleLayerNet):
    ...
    def activation(self, net_in):
        ...
        '''
        net_in_adj = np.exp(net_in - np.max(net_in, axis = 1, keepdims = True))
        return net_in_adj/np.sum(net_in_adj, axis = 1, keepdims = True)
    ...
    def loss(self, net_in, y, reg=0):
        ...
        return -np.sum(np.log(self.activation(net_in)[np.arange(net_in.shape[0]), y]))/net_in.shape[0] + reg/2*np.sum(np.square(self.wts))
    ...
    def gradient(self, features, net_act, y, reg=0):
        ...
        '''
        return ((features.T@(net_act - y)))/features.shape[0] + (reg*self.wts), np.sum(net_act - y, axis  = 0)/features.shape[0]
