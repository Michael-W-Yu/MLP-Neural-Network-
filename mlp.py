...
class MLP():
    ...
    def predict(self, features):
        ...
        y_net_act = y_net_in.copy()
        y_net_act[y_net_act<0] = 0
        return np.argmax(y_net_act@self.z_wts+ self.z_b, axis = 1)
    ...
    def forward(self, features, y, reg=0):
        ...
        z_net_in_adj = np.exp(z_net_in - np.max(z_net_in, axis = 1, keepdims = True))
        z_net_act = z_net_in_adj/np.sum(z_net_in_adj, axis = 1, keepdims = True)
        loss = -np.sum(np.log(z_net_act[np.arange(z_net_in.shape[0]), y]))/z_net_in.shape[0] + reg/2*np.sum(np.square(self.z_wts)) + reg/2*np.sum(np.square(self.y_wts))
    ...
    def fit(self, features, y, x_validation, y_validation,
        ...
        for i in range(n_iter):
            index = np.random.randint(num_samps, size=mini_batch_sz)
            features_batch = features[index, :]
            y_batch = y[index]
            y_in, y_act ,z_in, z_act, loss = self.forward(features_batch, y_batch, reg=reg)
            grads = self.backward(features_batch, y_batch, y_in, y_act ,z_in, z_act, reg=reg)
            loss_history.append(loss)
            self.y_wts = self.y_wts - lr*grads[0]
            self.y_b = self.y_b - lr*grads[1]
            self.z_wts = self.z_wts - lr*grads[2]
            self.z_b = self.z_b - lr*grads[3]
            ...
            if i % iter_per_epoch:
                train_acc_history.append(self.accuracy(y_batch, self.predict(features_batch)))
                validation_acc_history.append(self.accuracy(y_validation, self.predict(x_validation)))
